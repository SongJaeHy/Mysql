...
-- DML과는 달리 commit; 없이도 바로바로 서버에 반영됩니다.ㄹ

-- DCL : Data Control Language
-- 데이터 제어 언어로 특정 계정에 권한을 부여하거나 뺏을 때 사용합니다.
...
-- id오하 이름쪽에만 데이터를 넣을때는 컬럼명을 생략할 수 없습니다.
...
-- 모든 컬럼에 값을 입력하는 상화잉어도 컬럼명을 생략할 수 없다.
INSERT INTO testTbl1(userName, age, id) VALUES('송재현', 10,4);

SELECT * FROM testTbl1;

/* 지금 testTbl1같은경우 id값을 수동으로 입력해주고 있습니다.
...
-- AUTO_INCREMENT의 기중값은 1부터 1씩 자동으로 증가하지만
...
INSERT INTO testTbl2 VALUES (1, '양이', 4);
INSERT INTO testTbl2 VALUES (2, '옹이', 4);
INSERT INTO testTbl2 VALUES (3, '멍이', 4);
INSERT INTO testTbl2 VALUES (4, '멍멍', 3);
INSERT INTO testTbl2 VALUES (5, '멍뭉', 3);
...
INSERT INTO testTbl2 VALUES (null, '얼룩말', 2);

-- 하나의 INSERT INTO 구문으로 여러 row를 추가할 때는
-- VALUES 뒤 쪽을 연달아 작성합니다.

INSERT INTO testTbl3 VALUES (null, '알', 4),
(null, '오리',1), (null, '닭',2);
...
-- testTbl2의 userName을 전부 '소'로 바꾸는것
...
-- WHERE절을 시용해서 age를 필터링합니다.

UPDATE testTbl2 set userName = '개' WHERE age =4;



-- 이걸 응용해서 특정 컬럼의 값을 일괄적으로 계산식을 따라
...
-- DELETE FROM은 삭제시 시간이 오래걸리는데 트랜잭션 로그라는것을
-- 한 로우마다 일일이 작성하기 때문입니다.
...
-- 중복되지 않는다고 해도 시스템 싱 전체 데이터가
-- 입력되지 않는 문제가 발생합니다.
-- 따라서 이 떼 증복되는 1건은 중복된다면 무시하고
-- 이후 나머지 99개 데이터는 중단 없이 정상적으로
-- 입력하도록 만들 수 있습니다.
...
INSERT INTO memberTbl VALUES('bbk', '바비킴', '서울');
INSERT INTO memberTbl VALUES('liy', '이지은', '경기');
...
INSERT IGNORE INTO memberTbl VALUES('liy', '이지은', '경기');
INSERT IGNORE INTO memberTbl VALUES('jbj', '장범준', '천안');

-- 데이터를 집어넣되, 이미 존재하는 게이터면 데이터 갱신을 원하는 경우
INSERT INTO memberTbl values('liy', '이지금', '제주');
...
-- ON DUPLICATE KEY UPDATE를 이용해서 없는 요소로도 테스트
INSERT INTO memberTbl VALUES ('lkw', '이지', '서울')
ON DUPLICATE KEY UPDATE name = '지금', addr = '경기';
...
-- 올 초에도 DB를 날려먹느 사건이 유명 개발사에서도 나온 만큼
...
-- 1. 좌측의 navigator에서 Adminstration탭을 클릭합니다.
-- 2. Users and Privileges를 클릭합니다.
-- 3 열리는 화면에서 좌하단 Add Account를 클릭한 다음
-- Login name, passwoer 등을 입력합니다.
...
-- GRART aLL on *.* To manager@'%' WITH GRENT OPTION;  
-- 위 두 줄을 이용해 처리할 수도 있습니다.

-- manager2 계정을 커맨드라인 환경에서 만들기

-- 커맨드라인 접속시 mysql -u 계정명 -p 로 연결 가능
-- 접속 후 show databases; 를 이용해 DB목록 확인 가능
-- 계정 목록을 볼 때는 1. use mysql;
...
-- 계정 만들기
-- CREATE USER 아이디@'%' IDENTIFIRED BY '비밀번호';

-- 계정에 권한 주기
-- GRANT 권한1, 권한2.. ON DB명.테이블명 RO 아이디@'%';
...
-- REVOKE 권한들... ON 디비명.테이블명 FROM 계정명;
...
SELECT buytbl.num, buyTBl.userID, buyTbl.prodName, buyTbl.groupName, buyTbl.price,
buyTbl.amount, userTbl.name, userTbl.addr, userTbl.pnum FROM buyTbl INNER JOIN userTbl
ON buyTbl.userID = userTbl.userID;

-- FROM 테이블명 별명을 적을 경우 테이블명을 풀네임으로 적지 않고
-- 별명으로 대체할 수 있습니다.
SELECT buytbl.num, buyTBl.userID, buyTbl.prodName, buyTbl.groupName, buyTbl.price,
buyTbl.amount, userTbl.name, userTbl.addr, userTbl.pnum FROM buyTbl b INNER JOIN userTbl
 u ON buyTbl.userID = userTbl.userID;

-- WHERE절은 먼저 결과가 나온 상태에서 추가 필터링만 담당합니다.
SELECT b.num, b.userID, b.prodName, b.groupName, b.price,
b.amount, u.name, u.addr, u.pnum FROM buyTbl b INNER JOIN userTbl
 u ON b.userID = u.userID WHERE amount > 1;
 
 -- 위 구문에 ORDER BY를 추가해 비깐 가격의 물건부터 나열해주세요.
 SELECT b.num, b.userID, b.prodName, b.groupName, b.price,
 b.amount, u.name, u.addr, u.pnum FROM buyTbl b INNER JOIN
 userTbl u ON b.userID = u.userID WHERE amount > 1 
...
-- 왼쪽 데이블 데이터는 매칭이 되는 데이터만 출력합니다. 
SELECT * FROM buyTbl b
	LEFT OUTER JOIN userTbl u
	ON b.userID = u.userID;


/* FULL HOIN은 누락데이터 없이 양쪽 테이블의 모든 자료를 보여줍니다.
...
INSERT INTO student VALUES ('송재현', '서울');
INSERT INTO student VALUES ('류현진', '경기');
INSERT INTO student VALUES ('스프링', '제주');

INSERT INTO membership VALUES ('송재현', 100);
INSERT INTO membership VALUES ('류현진', 200);
INSERT INTO membership VALUES ('어타니', 50);
