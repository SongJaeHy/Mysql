...
먼저 변수 지정 빛 출력부터 진행하겠습니다.*/

-- <MySQL에서 변수 사용하기>

use sqldb;

-- MYSQL에서 변수를 지정할때는 SET @변수명 = 깂; 의 문법을 씁니다.
SET @myVar1 = 5;
SET @myVar2 = 3;
SET @myVar3 = 4.25;
SET @myVar4 = '가수의 이름 =>';
...
-- SELECT그믄이 그렇듯, 콤마(,)를 이용해서
-- 출력데이터를 여러 개 나열할 수도 있습니다.
...
-- PREPARE 구문
-- PRAPARE 구문은 가변적으로 들어갈 문장요소 자리를
-- ?로 구멍을 뚫어놓고, 그 자리를 채우는 방식으로 만듭니다.
-- 사용법은
-- PREPARE 구문이름
-- 	FROM '실제 쿼리문';
-- 방식으로 선언하고
-- 호출은 EXECUTE 구문이름 USING 잔딜뱐슈;
-- 로 호출합니다.
...
-- <MySQL 내장함수>
-- COMCAT, CAST, CONVERT 등과 깉이
-- 내부에 이미 선언되어있어서 바로 호출해서 쓸 수 있는
-- 함수들을 보겠습니다.

-- if(수식, 참일때리턴, 거짓일때린턴)
-- 삼항ㅇ연산자와 비슷하게 판간합니다.
SELECT IF(100 > 200, '참입니다', '거직입니다');

-- IFNULL(수식1, 수시2)
-- 수식 1이 NULL이 아니면 수식 1이 반환, 수식1이 NULL이면
-- 수식 2로 반환함
SELECT IFNULL(NULL, '널입니다'), IFNULL(100, '널입니다');

-- NULLIF(수식1, 수식2)
-- 스식1과 수식2가 같으면 NULL을 반환하고 다르면 수식 2이 반환
...
-- CHAR()는 workbench상에 버그로 인해 모든 문자가 BLOB로
-- 표현되고 있습니다.
-- 원래 값을 보려면 BLOB -> 우클릭 -> 
...
-- 이때 결과 나오는 숫자는 문자열의 길이입니다.
SELECT CHAR_LENGTH('가나다라마바사');
SELECT CHAR_LENGTH('abcdefgjkelaga');

-- CONCAT(문자열1, 문자열2.....);
SELECT CONCAT('가', '다', '마', '바', '사');
...
-- 믄자열을 길이 만큼 늘려놓고 빈 칸에 채울 문자여을 채웁니다. 
SELECT LPAD('이것이', 5, '#-'), RPAD('저겅이', 6, '#~');
...
-- 모든 공백을 다 삭제해줍니다.
-- 공백이 아닌 특정 문자를 삭제하도록 구문을 지정할 수 있습니다.
SELECT TRIM('      무야호      ');
-- 방향은 BOTH(양쪽), LEADING(왼쪽), TRAILING(오른쪽)
-- 중 하나를 고르면 됩니다.
SELECT TRIM(BOTH 'ㅋ' FROM 'ㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㄹㅇㅋㅋㅋ');
...
-- 찾아 바꾸기 입니다.
SELECT REPLACE('JAVA호 작성되었습니다. JAVA', 'JAVA', 'MySQL');


-- REVERSE (문자열)
-- 문자여을 인덱스 역순으로 재배치해줍니다.
...
-- use sqldb 를 해 주시고
-- usertbl에서 name, addr, height흫 조회하는
...
BEGIN
    ...
    DECLARE days INT; # 오늘닐찌 - 입사일 해서 경과일수
...
-- 두 번째, 세 번째 조건... 을 줄줄이 붙일 수 있습니다.
-- getScore 프로시저르 생성해주세요.
-- 변수로 point(int) 는 점수를 입력받는데 77점을 주시고
-- 변수로 ranking(char)는 90점 이상이면 'A'를 저장받고
-- 80점 이상이면 'B', 70점 이상이면 'C', 60점 이사이면 'D'
-- 그 이하 점수는 'F'를 저장받습니다.
...
-- 기본적인 변수나 점구대는 모두 getScore() 프로시저에
...
-- 3 + 6 + 9 + 12 ... 와 같은 방식으로
-- 3부터 99까지 더했을때 총합이 몇인지 SELECT문으로
...
-- 테이블을 수정할때는 ALTER TABLE 구문을 추가합니다.
-- 컬럼 추가
-- ALTER TABLE 체이블명 ADD 컬럼명 자료형(크기)
-- 		DEFAULT 디폴트값;
-- 기존 테이블릐 가장 마지막 컬럼 다음 위치에 생성됩니다.
-- job 컬럼을 wbtbl에 추가해보겠습니다.
-- job 컬럼은 varchar(15) 자료를 저장받고
-- 아무것도 입력받지 않았을때, '0' 을 입력받습니다.(디폴트값설정)
use sqldb;
...
-- 컬럼 삭제
-- ALTER TABLE 체이블명 DROP COLUMN 컬럼명;
...
-- 주면 됩니다.
ALTER TABLE wbtbl CHANGE COLUMN job ujob 
...
-- (CONSTRAINT) 제약조건;
ALTER TABLE wbtbl MODIFY  ujob varchar(20)
NOT NULL; 
