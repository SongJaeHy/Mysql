...
-- emplyoees 데이터베이스 내부의 employees table 조회
...
DROP DATABASE sqlDB; -- sqlDB가 존재시 삭제
...
-- primary key(주요 키)
-- 각 테이블을 대표할 수 있는 데이터를 저장하는 컬럼에 붙인다.
-- 모든 테이블은 무조건 하나의 primary key를 갖는다.
-- primary key는 자동으로 중복을 방지하고, null도 받을수없다.
...
-- foreign key를 거는 컬럼의 자료형과 크기는 양 쪽 모두에서 같아야 한다.
-- 참조컬럼에 있는 자료만 타켓컬럼으로 들어올 수 있다.
-- 지금같은경우 buyTbl의 userId에는 userTbl의 userID에 존재하는
-- 데이터만 저장할 수 있다.
CREATE TABLE buyTbl(
...
INSERT INTO buyTbl VALUES(null, 'BBK', '운동화', NULL, 30, 2);
INSERT INTO buyTbl VALUES(null, 'BBS', '운동화', NULL, 20, 1);

SELECT * FROM userTbl;
SELECT * FROM buyTbl;

-- 여태까지의 SELECT구문은 조건 없이 모든 데이터를 조회했습니다.
-- 극단적인경우 employees 테이블의 수십만 row를 다 조회하다보니
...
-- 1970년 이후 출생했으며, 키가 182 이상인 아이디와 이름을
-- 조회하는 구문을 아래에 작성해주세요.

SELECT userID, name FROM userTbl WHERE (birthYear>1970) and (height>=182);

-- 키가 180이상, 183이하인 사람만 필터링해주세요.
SELECT * FROM userTbl WHERE (height >= 180) and (height <= 183);

-- 위와 같이 범위를 정하는 구문은 BETWEEN ~ AND로 간단하게 구현합니다.
...
-- 특정컬럼에 괄호에 담긴 데이터가 포함되는 경우를 전부 출력합니다.
SELECT * FROM userTbl WHERE addr in('경남','전남','경북');

-- like 연산자는 일종의 표현 양식을 만들어줍니다.
-- like 연산자를 이용하면 %라고 불리는 와이드 카드나 혹은 _라고 불리는
...
-- 서브 쿼리(하위쿼리)란 1차적 결과를 얻어놓고, 거기서 다시
...
-- 키가 더 큰 타 지역 사람을 구하시오.(max를 활용하세요)
...
-- 아래 구문은 170, 173을 동시에 반황해서 에러가 발생함
SELECT * FROM userTbl WHERE height >=
(SELECT height FROM userTbl WHERE addr = '경남');

 -- ANY구문을 사용하면 (170보다 크거나 같은)  OR (173보다 크거나 같은)
 -- 조건을 들어갑니다(개별 값 하나하나에 대해 OR이 걸림)
 -- 즉 170보다 크거나 같다로 결론이 나옵니다.
 SELECT * FROM usertbl WHERE height >= ANY
 (SELECT height FROM usertbl WHERE addr='경남');
 
 -- ALL구문을 사용하면 (170보다 크거나 같은) AND (173보다 크거나 같은)
 -- 조건으로 들어갑니다. (개별 조건 하나하나에 대해 AND 걸림)
 -- 즉 173보다 크거나 같다로 결론이 나옵니다.
 SELECT * FROM usertbl WHERE height >= ALL
 (SELECT height FROM usertbl WHERE addr='경남');
 
 -- ANY를 사용함에 있어서 = 로 조건비교를 하는 경우는
 -- 사실상 in 키워드와 같은 효과를 냅니다.
 SELECT Name, height FROM userTbl WHERE height = ANY
 (SELECT height FROM userTbl WHERE addr='경남');
 
 -- ORDER BY는 결과물의 개수나 종류에는 영향을 미치지 않지만
 -- 결과물을 순서대로(오름차순, 내림차순) 정렬하는 기능을 가집니다.
 -- SELECT 컬럼명 FROM 테이블명 ORDER BY 기준컬럼이 정렬기준;
 -- 정렬기준은 ASC(오름차순), DESC(내림차순)이 있으며
 -- 입력이 없을 경우 기본은 ASC입니다.
 -- 가입한 순서(mDate순으로 오름차순 조회를 해 주세요.)

SELECT * FROM userTbl ORDER BY mDate DESC;

-- 정렬시 동점인 부분을 처리하기 위해 order by절을 두 개 이상
-- 동시에 나열할 수도 있습니다.
-- 아래 코드는 키로 내림차순을 하되, 동점이면 가나다라순으로 나열합니다.
...
-- 데이터가 300000개에 가깝다보니 모든 데이터를 보여주지 않ㅎ고
-- 1000개만 보여주도록 설정되어 있음.
SELECT * FROM employees;

-- 테이블명 뒤에 limit 숫자; 가 오는 경우는 적은 숫자 개수만큼만
-- 결과창에 보여줍니다.
-- 하나만 적으면 자동으로 0번부터 n개를 보여줍니다.
...
-- SELECT 컬럼명, 집계함수(컬럼명2)... FROM 테이블명 GROUP BY
-- 묶을 컬럼명;
...
-- GROUP BY절에서는 조건절로 WHERE을 쓰지 않습니다.
